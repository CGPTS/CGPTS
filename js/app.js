...
const CONFIG = Object.freeze({
  // Web3Forms sends the committee notification email.
  ...
  COMMUNITY_WHATSAPP_MANAGER: '972503088956',
  COMMUNITY_MANAGER_NAME:     'לירז לוי',
  DRAFT_STORAGE_KEY:   'ms-hall-booking-draft-v1',
  DRAFT_ACTIVE_KEY:    'ms-hall-booking-active-draft-v1',
  DRAFT_TAB_ID_KEY:    'ms-hall-booking-tab-id',
  DRAFT_VERSION:       1,
...
function restoreDraft(draft) {
  ...
  const isBlockedCommunityDraft = normalizedEventType === 'אירוע קהילתי';
  ...
  currentStep = isBlockedCommunityDraft
    ? 3
    : Math.min(Math.max(parseInt(draft.currentStep, 10) || 2, 2), 6);
  AppState.set('currentStep', currentStep);
  showStep(currentStep);
  updateProgressBar();
  showCommunityBookingNotice(isBlockedCommunityDraft);
  ...
  saveDraft();
  if (isBlockedCommunityDraft) {
    Toast.warning('אירוע קהילתי אינו ניתן לשריון במערכת. יש לפנות ללירז לוי בוואטסאפ.');
  } else {
...
async function downloadEventDocumentAndUnlock() {
  // Backward compatibility for old cached HTML: the final step now submits
  // the booking and shows the confirmation screen.
  return submitOrder();
}
...
function openCommunityBookingWhatsApp() {
  const msg = `שלום ${CONFIG.COMMUNITY_MANAGER_NAME}, אני מעוניין/ת לתאם אירוע קהילתי באולם מרכז שפירא.`;
  window.open(
    `https://wa.me/${CONFIG.COMMUNITY_WHATSAPP_MANAGER}?text=${encodeURIComponent(msg)}`,
    '_blank',
    'noopener,noreferrer'
  );
}

function showCommunityBookingNotice(show = true) {
  const notice = document.getElementById('communityBookingNotice');
  if (notice) notice.style.display = show ? 'flex' : 'none';
}

function blockCommunityBooking() {
  showCommunityBookingNotice(true);
  FieldValidator.setError('eventType', 'אירוע קהילתי מתואם ישירות מול לירז לוי בוואטסאפ');
  Toast.warning('לא ניתן לשריין אירוע קהילתי דרך המערכת. יש לפנות ללירז לוי בוואטסאפ.');
  document.getElementById('communityBookingNotice')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
  return false;
}
  ...
  switch (step) {
    ...
    case 3: {
      ...
      if (!event) {
        ...
        valid = false;
      } else if (normalizeEventType(event) === 'אירוע קהילתי') {
        blockCommunityBooking();
        valid = false;
      }
...
async function submitOrder() {
  ...
  if (normalizedEventType === 'אירוע קהילתי') {
    currentStep = 3;
    AppState.set('currentStep', currentStep);
    showStep(currentStep);
    updateProgressBar();
  ...
  const primarySubmitBtn = document.getElementById('downloadBtn');
  const secondarySubmitBtn = document.getElementById('step7SubmitBtn');
  [primarySubmitBtn, secondarySubmitBtn].forEach((btn) => {
    if (!btn) return;
    btn.disabled = true;
  ...
  try {
    ...
    // reserved — a failure in the notification step must never undo it
    // or block a retry. We remember that the booking is already saved so a
    // retry skips straight to the committee notification.
    if (!AppState.get('bookingSaved')) {
      ...
      Logger.info('Booking already saved — skipping Firestore, retrying committee notification only.');
    }
    ...
    try {
      Logger.info('Sending committee email via Web3Forms…');
      await withRetry(() => sendCommitteeEmail(emailParams));
      Logger.success('Committee email sent (Web3Forms)');
      Toast.success('ההזמנה נקלטה בהצלחה! פרטי ההזמנה נשלחו לוועד.');
    } catch (emailError) {
      Logger.error('submitOrder: committee email failed (booking already saved)', emailError);
      showCommitteeWhatsAppFallback({ orderNumber, mergedOrder, calInfo, priceText, eventType: normalizedEventType });
      Toast.warning('ההזמנה נשמרה בהצלחה, אך לא הצלחנו לשלוח מייל אוטומטי לוועד. שלחו את הסיכום בוואטסאפ כדי שהוועד יקבל אותו.', 9000);
    }
    ...
    if ((AppState.get('currentStep') || currentStep) === 7) {
      [primarySubmitBtn, secondarySubmitBtn].forEach((btn) => {
        if (!btn) return;
        btn.disabled = false;
        btn.textContent = btn.dataset.originalText || 'שלח הזמנה ←';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('eventType')?.addEventListener('change', () => {
    const communitySelected = isCommunityEventSelected();
    showCommunityBookingNotice(communitySelected);
    if (communitySelected) {
      FieldValidator.clearError('eventType');
      Toast.warning('אירוע קהילתי מתואם ישירות מול לירז לוי בוואטסאפ.');
    }
    syncMemorialPricingOptions();
    const step = AppState.get('currentStep') || currentStep;
    if (step >= 4) {
